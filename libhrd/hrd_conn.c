...
#ifdef HRD_RoCE
struct hrd_ctrl_blk*
hrd_ctrl_blk_init(int local_hid, int port_index, int gid_index, 
...
#endif
{
    ...
    if(num_conn_qps >= 1) {
        ...
        if(prealloc_conn_buf == NULL) {
            ...
            /* the same memory buffer can be registered by different 
             * control blocks with different ib_flags
             */
...
void hrd_create_conn_qps(struct hrd_ctrl_blk *cb)
{
    ...
    for(i = 0; i < cb->num_conn_qps; i++) {
        ...
        create_attr.cap.max_send_wr = HRD_Q_DEPTH;
        /* We don't do RECVs on conn QPs */
        // create_attr.cap.max_recv_wr = 1;
        /* Allen: why don't do recvs on conn QPS? */
        create_attr.cap.max_recv_wr = HRD_Q_DEPTH;	
        create_attr.cap.max_send_sge = 1;
        create_attr.cap.max_recv_sge = 1;
        create_attr.cap.max_inline_data = HRD_MAX_INLINE;
	...
	int n, struct hrd_qp_attr *remote_qp_attr)
{
    ...
    conn_attr.ah_attr.grh.sgid_index = cb->gid_index;
